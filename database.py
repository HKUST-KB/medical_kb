...
class Neo4j():
    ...
    def __init__(self):
        self.graph = Graph("http://localhost:7474",
                           username="neo4j", password="kbcdata")

    def insert(self):
        a = Node('Person', name=['A','L'])
        self.graph.create(a)
...
class rdfstore():
    ...
    def __init__(self, endpoint="http://localhost:5820/test"):
        self.endpoint = endpoint

    def search(self, rq):
        # sparql = SPARQLWrapper(self.endpoint+"/query")
        # sparql.setQuery("""
        #     SELECT ?s
        #     WHERE { ?s ?p ?o .}
        #     LIMIT 3
        ...
        rq = """
            ...
            """
        endpoint = 'http://localhost:5820/test/query'
    ...
    def insert_kb_into_stardog(self, filename, format="nt"):
        g = Graph()
        g.parse(filename, format=format)
        g.serialize(format=format)
        print("graph has %s statements." % len(g))
        allnumber = len(g)
        sparql = SPARQLWrapper(self.endpoint + "/update")
        i = 0
        queryString = u"INSERT DATA {\n"
        for s, p, o in g:
            # print s.n3(),p.n3(),o.n3()
            # if o.__class__ is Literal and o.datatype == year_type:
            #     continue
            queryString = queryString + \
                unicode(s.n3()) + ' ' + unicode(p.n3()) + \
                ' ' + unicode(o.n3()) + '.\n'
            i += 1
            # print queryString
            if i > 1000:
...
if __name__ == '__main__':
    db = Neo4j()
    # db.insert()
    print len(db.graph.nodes)
    print db.matchHudongItembyTitle("河南省")
    # g = rdfstore()
    # print g.search("")
